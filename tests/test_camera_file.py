...
def test_reads_one_frame_per_interval(sample_video: Path) -> None:
    """A 3 second clip at a 1 second interval yields 3 frames."""
    with FileSource(sample_video, interval_seconds=1.0) as source:
        frames = list(iter(source.read, None))

    assert len(frames) == 3


def test_a_shorter_interval_yields_more_frames(sample_video: Path) -> None:
    with FileSource(sample_video, interval_seconds=0.5) as source:
        frames = list(iter(source.read, None))

    assert len(frames) == 6


def test_replay_does_not_wait_for_wall_clock_time(sample_video: Path) -> None:
    """The whole point: a long recording must replay in a moment, not in hours."""
    import time

    started = time.monotonic()
    with FileSource(sample_video, interval_seconds=1.0) as source:
        list(iter(source.read, None))

    assert time.monotonic() - started < 1.0


def test_timestamps_follow_the_recording_timeline(sample_video: Path) -> None:
    with FileSource(sample_video, interval_seconds=1.0) as source:
        frames = list(iter(source.read, None))
...
def test_reading_before_open_is_an_error(sample_video: Path) -> None:
    with pytest.raises(VideoSourceError, match="not open"):
        FileSource(sample_video, interval_seconds=1.0).read()
